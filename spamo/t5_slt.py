...
def build_encoder(
    cfg: Dict,
) -> "RecurrentEncoder | TransformerEncoder | DeformableTransformerEncoder":
    """
...
class FlanT5SLT(AbstractSLT):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        print("===" * 40)
        print("[MODEL CONFIG] fusion_mode:", self.fusion_mode)
        print(
            "[MODEL CONFIG] use_temporal_conv:",
            self.use_temporal_conv,
            "| use_second_encoder:",
            self.use_second_encoder,
        )
        print(
            "[MODEL CONFIG] cross_modal_align:",
            self.cross_modal_align,
            "| combined_loss:",
            self.combined_loss,
        )
        print(
            "[MODEL CONFIG] use_in_context:",
            self.use_in_context,
            "| num_in_context:",
            self.num_in_context,
        )
        print(
            "[MODEL CONFIG] use_data_augmentation:",
    ...
    def prepare_models(self, t5_model: str) -> None:
        ...
        self.spatio_proj = build_vision_projector(
            "linear", self.input_size, self.inter_hidden
        )
        ...
        self.pose_proj = build_vision_projector(
            "linear", self.pose_input_size, self.inter_hidden
        )
        self.fusion_proj = build_vision_projector(
            "mlp2x_gelu", self.inter_hidden, self.t5_model.config.hidden_size
        )
        ...
        if self.use_second_encoder:
            print(
                f"[SecondEncoder] Building encoder type='{self._encoder_cfg.get('type', 'recurrent')}' "
    ...
    def prepare_inputs(
        ...
        prompts = [p.format(lang) for p, lang in zip(prompts, samples["lang"])]
    ...
    def prepare_visual_inputs(self, samples: Dict) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if spatial:
            ...
            if (
                self.training
                and hasattr(self, "use_data_augmentation")
                and self.use_data_augmentation
            ):
            ...
            spatial_mask = create_mask(
                seq_lengths=samples["num_frames"], device=self.device
            )

        # Process spatiotemporal features
        if spatiotemporal:
            spatiotemporal_outputs = pad_sequence(
                samples["glor_values"], batch_first=True
            )
            ...
            if (
                ...
                spatiotemporal_outputs = self.augmenter(
                    spatiotemporal_outputs, samples["glor_lengths"]
                )
        ...
        if pose:
            ...
            if len(pose_values_local) > 0:
                ...
                pose_padded = (
                    pad_sequence(pose_values_local, batch_first=True)
                    .to(self.device)
                    .float()
                )
        ...
        if self.fusion_mode == "joint":
            ...
            for i in range(bs):
                ...
                if spatiotemporal:
                    parts.append(
                        spatiotemporal_outputs[i, : spatiotemporal_length[i], :]
                    )
                if pose:
            ...
            if self.use_temporal_conv:
                ...
                visual_masks = create_mask(
                    seq_lengths=new_length.tolist(), device=self.device
                )
            ...
            fused_outputs = self.adaptive_fusion(
                spatial_outputs, spatiotemporal_outputs, pose_outputs
            )
            ...
            if spatial:
                ...
                if self.use_temporal_conv:
                    ...
                    visual_masks = create_mask(
                        seq_lengths=pose_conv_outputs["feat_len"]
                        .to(torch.int)
                        .tolist(),
                        device=self.device,
                    )
                    ...
                    visual_masks = create_mask(
                        seq_lengths=pose_lengths, device=self.device
                    )
            ...
            if self.fusion_mode == "spatiotemporal":
                ...
                if self.use_temporal_conv:
                    ...
                    visual_outputs = active_outputs
                    visual_masks = create_mask(
                        seq_lengths=active_lens, device=self.device
                    )
    ...
    def shared_step(
        self, inputs: Dict, split: str, batch_idx: int
        ...
        if self.cross_modal_align:
            if self.warm_up_steps is None and not self.combined_loss:
                with torch.no_grad():
                    input_embeds, input_masks, output_tokens, targets = (
                        self.prepare_inputs(
                            visual_outputs, visual_masks, inputs, split, batch_idx
                        )
                    )

                cont_loss = self.visual_textual_align(
    ...
    def configure_optimizers(self):
        ...
        print(
            f"--> Optimizer Setup: Total Steps={total_steps}, Warmup Steps={warmup_steps}"
        )
