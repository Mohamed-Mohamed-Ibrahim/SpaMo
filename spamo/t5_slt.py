    ...
    def prepare_visual_inputs(self, samples: Dict) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.fusion_mode == 'joint':
            ...
            if self.use_temporal_conv:
                ...
                feat = joint_outputs
                new_feat_lens = new_length.to(self.device).long()
                visual_outputs = feat
            ...
            if self.use_temporal_conv:
                ...
                feat = fused_outputs
                new_feat_lens = torch.tensor(samples['num_frames'], device=self.device).long()
                visual_outputs = feat
            ...
            if spatial:
                ...
                if self.use_temporal_conv:
                    ...
                    feat = pose_outputs
                    new_feat_lens = torch.tensor(pose_lengths, device=self.device).long()
                    visual_outputs = feat
